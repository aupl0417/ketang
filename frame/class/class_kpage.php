...
class kpage {
    ...
    protected $rollPage;
    // 分页显示定制
    public $config = array('preview' => '上一页    ', 'next' => '下一页');
    ...
    protected function getLink($page, $text) {
        switch ($this->method) {
            ...
            case 'html':
                ...
                break;
            case 'xuelist':
                return '<a href="' . $this->getUrl($page) . '">' . $text . '</a>';
                break;
            default:
    ...
    protected function showDefault() {
        ...
        $pageString = $linkPage."<li><span>共有：$this->totalRows 条记录,每页：$this->listRows 条</span></li>";
        return $pageString;
    }

    protected function showNoInfo() {
        $pageString = "";
        $linkPage = "";
        if ($this->totalPages <= 1) {
            return false;
    ...
    protected function showXuelist() {
        $pageString = "";
        $linkPage = "";
        if ($this->totalPages <= 1) {
            return false;
        }
        $linkPage .= $this->xuepreviewPage('上一页');
        $linkPage .= $this->xuenextPage('下一页');
        $pageString = $linkPage;
        return $pageString;
    }
    ...
    protected function nextPage($name = '') {
        ...
        if ($this->nowPage < $this->totalPages) {
            ...
            return $this->getLink($this->nowPage , $name);
        }
    }
     ...
     * @return string
     */
    protected function xuepreviewPage($name = '') {
        if ($name == '') {
            $name = $this->config['preview'];
        }
        if ($this->totalRows != 0 && $this->nowPage>1) {
            return $this->getLink($this->nowPage - 1, $name);
        }else{
            return "<a class='current'>$name</a>";
        }
    }
     ...
     *
     */
    protected function xuenextPage($name = '') {
        if ($name == '') {
            $name = $this->config['next'];
        }
        if ($this->nowPage < $this->totalPages) {
